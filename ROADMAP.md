...
### AA. Dashboard Internationalization — FOUNDATION SHIPPED, FULL TRANSLATION DEFERRED TO v1.1+
- **Status:** Phase 1 (foundation) is shipped. The remaining ~1,600-string extraction is deliberately deferred past v1.0. Full spec lives in `tasks/todo.md` and `tasks/i18n-sprint-plan.md`
- **What shipped:** A private I18n backend isolated from the host app, request-scoped locale state, the `red_t` helper, plural/relative-time/date-format helpers, and a dynamic `<html lang>`. `config.dashboard_locale` now drives RED's own translation lookups, not just Pagy labels. **Zero user-visible change** — every string is still English, but the machinery to translate them exists and is tested
- **What's deferred:** Extracting ~1,600 strings across 48 views, inline JS, mailers, and notification payloads, plus shipping non-English locales
- **Why deferred:** Two reasons. First, **depth before breadth** — the same principle that defers APM (item Z). Translation is reach, not depth; it doesn't draw on the in-process advantage that is the gem's actual moat. A German-language dashboard is not why someone picks RED over Sentry; local variables at raise point is. Second, **i18n is irreversible surface area** — once translations ship, every new feature costs keys in every locale file forever, and the project owns translation quality in languages the maintainer doesn't read. Taking that tax on before v1.0 would slow every remaining v1.0 feature. After v1.0 the string surface stops moving and the cost is predictable
- **Why the foundation shipped anyway:** It is 2-3 days, carries zero user-visible risk, and makes `config.dashboard_locale` honest (it previously promised more than it delivered). It also makes the remaining work incremental — pages can be translated one release at a time rather than in a single 20-day push — and de-risks the estimate by proving what extraction actually costs
- **Demand signal:** No user has requested this. Zero i18n issues filed. Revisit when demand appears; the foundation means shipping a locale is then days, not weeks
- **Effort:** Foundation 2-3 days (done) · full extraction 16-20 days (deferred)
