...
### AA. Dashboard Internationalization — EXTRACTION SHIPPED, LOCALES IN PROGRESS
- **Status:** Phases 1-6 of `tasks/i18n-sprint-plan.md` are shipped or in progress on `feat/i18n-layout-nav`. The ~1,600-string extraction that this item previously deferred is **done**: roughly 1,500 keys across views, inline JS, mailers and notification payloads now render through `red_t`. Full spec lives in `tasks/todo.md` and `tasks/i18n-sprint-plan.md`
...
- **What remains:** Shipping `de`, `fr`, `es` and `pt-BR` (machine-translated, explicitly unreviewed — the maintainer reads only English), then the Phase 7 verification and release pass
...
- **Effort:** Foundation 2-3 days (done) · extraction + tooling ~14 days (done) · locales and release (in progress)
