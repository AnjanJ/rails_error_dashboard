...
class CreateRailsErrorDashboardCompleteSchema < ActiveRecord::Migration[7.0]
  def up
    ...
    # Mark that we're using the squashed migration path
    # This prevents incremental migrations from running
    create_table :rails_error_dashboard_schema_markers do |t|
  ...
  def down
    ...
    return unless table_exists?(:rails_error_dashboard_schema_markers)
    ...
    drop_table :rails_error_dashboard_schema_markers, if_exists: true
  end
end
