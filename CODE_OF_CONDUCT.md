# Code of Conduct

## Our Pledge

We as members, contributors, and leaders pledge to make participation in our community a harassment-free experience for everyone, regardless of age, body size, visible or invisible disability, ethnicity, sex characteristics, gender identity and expression, level of experience, education, socio-economic status, nationality, personal appearance, race, caste, color, religion, or sexual identity and orientation.

We pledge to act and interact in ways that contribute to an open, welcoming, diverse, inclusive, and healthy community.

---

## Our Standards

### Positive Behavior ✅

Examples of behavior that contributes to a positive environment:

- **Being respectful** - Treating all community members with respect and kindness
- **Being welcoming** - Helping newcomers feel comfortable and supported
- **Being constructive** - Providing helpful feedback and suggestions
- **Being collaborative** - Working together toward common goals
- **Being patient** - Understanding that everyone has different skill levels
- **Giving credit** - Acknowledging others' contributions
- **Accepting feedback** - Being open to constructive criticism
- **Focusing on what's best** - Prioritizing the community and project

### Unacceptable Behavior ❌

Examples of unacceptable behavior:

- Trolling, insulting/derogatory comments, or personal attacks
- Public or private harassment
- Publishing others' private information without permission
- Sexualized language, imagery, or unwelcome advances
- Other conduct which could reasonably be considered inappropriate in a professional setting
- Sustained disruption of discussions or development
- Deliberate intimidation or stalking
- Advocating for or encouraging any of the above behaviors

---

## Our Responsibilities

### Project Maintainers

Project maintainers are responsible for:

- Clarifying standards of acceptable behavior
- Taking appropriate and fair corrective action
- Removing, editing, or rejecting comments, commits, code, issues, and other contributions that don't align with this Code of Conduct
- Temporarily or permanently banning contributors for behaviors deemed inappropriate, threatening, offensive, or harmful

### All Community Members

All community members are responsible for:

- Reading and understanding this Code of Conduct
- Treating others with respect and kindness
- Reporting violations when observed
- Supporting a healthy community environment

---

## Scope

This Code of Conduct applies within all community spaces, including:

- GitHub repositories (issues, PRs, discussions, wiki)
- Official communication channels
- Social media with official project accounts
- Community events (online or in-person)
- Private communications representing the project

This Code of Conduct also applies when representing the project in public spaces, such as using an official project email address, posting via official social media accounts, or acting as a representative at events.

---

## Enforcement

### Reporting

Instances of abusive, harassing, or otherwise unacceptable behavior may be reported to the project team at:

**Email:** anjan.jagirdar+red@gmail.com

All complaints will be:
- Reviewed and investigated promptly and fairly
- Kept confidential
- Responded to within 48 hours

### What to Include in a Report

When reporting, please include:

- Your contact information
- Names (real, nicknames, or pseudonyms) of involved parties
- Description of the incident
- Any supporting documentation (screenshots, links, etc.)
- Whether the incident is ongoing
- Any other relevant context

### Enforcement Process

1. **Receipt** - Acknowledgment within 24 hours
2. **Review** - Investigation and assessment (1-7 days)
3. **Decision** - Determination and action plan
4. **Notification** - Involved parties notified of outcome
5. **Appeal** - Option to appeal within 7 days

---

## Enforcement Guidelines

### 1. Correction

**Community Impact:** Use of inappropriate language or other behavior deemed unprofessional or unwelcome.

**Consequence:** A private, written warning providing clarity around the nature of the violation and an explanation of why the behavior was inappropriate. A public apology may be requested.

### 2. Warning

**Community Impact:** A violation through a single incident or series of actions.

**Consequence:** A warning with consequences for continued behavior. No interaction with the people involved for a specified period. This includes avoiding interactions in community spaces and external channels. Violating these terms may lead to a temporary or permanent ban.

### 3. Temporary Ban

**Community Impact:** A serious violation of community standards, including sustained inappropriate behavior.

**Consequence:** A temporary ban from any interaction or public communication with the community for a specified period. No public or private interaction with the people involved is allowed during this period. Violating these terms may lead to a permanent ban.

### 4. Permanent Ban

**Community Impact:** Demonstrating a pattern of violation of community standards, including sustained inappropriate behavior, harassment, or aggression toward individuals or groups.

**Consequence:** A permanent ban from any public interaction within the community.

---

## Examples in Practice

### Good Examples 👍

**Giving Feedback:**
- ✅ "This approach might have performance issues at scale. Have you considered using X instead? Here's why..."
- ❌ "This code is terrible. Anyone with experience would know better."

**Disagreeing:**
- ✅ "I understand your point, but I have a different perspective. Here's my reasoning..."
- ❌ "You're wrong. That's a stupid idea."

**Asking Questions:**
- ✅ "I'm not familiar with this pattern. Could you explain how it works?"
- ❌ "Why would anyone do it this way? This makes no sense."

**Welcoming Newcomers:**
- ✅ "Welcome! Don't worry if this seems complex at first. We're here to help. Check out our contributing guide to get started."
- ❌ "Read the docs before asking basic questions."

---

## Specific Guidelines

### Code Reviews

- Focus on code, not people
- Explain *why*, not just *what* to change
- Suggest alternatives with reasoning
- Appreciate good work
- Be patient with learning curves

### Issue Discussions

- Stay on topic
- Be constructive
- Assume good intentions
- Provide context and examples
- Search before posting duplicates

### Pull Requests

- Follow contribution guidelines
- Respond gracefully to feedback
- Keep discussions professional
- Close unneeded PRs promptly

---

## Questions?

If you have questions about this Code of Conduct:

- Open a [GitHub Discussion](https://github.com/AnjanJ/rails_error_dashboard/discussions)
- Email: anjan.jagirdar+red@gmail.com
- Reach out to project maintainers

---

## Attribution

This Code of Conduct is adapted from:
- [Contributor Covenant](https://www.contributor-covenant.org/), version 2.1
- [Ruby Community Conduct Guideline](https://www.ruby-lang.org/en/conduct/)
- Rails Code of Conduct

---

## Updates

This Code of Conduct may be updated periodically. Significant changes will be announced in project updates. Community members are expected to follow the current version.

**Version:** 1.0
**Last Updated:** January 13, 2026
**Effective Date:** January 13, 2026

---

## Commitment

The Rails Error Dashboard team is committed to providing a welcoming and inspiring community for all. We will not tolerate any form of harassment or discrimination. Together, we can build an amazing project and community.

Thank you for helping make this a welcoming, friendly community for everyone! 🙏
